...
### ✅ Interactive UI — 4 Result Sections

The Gradio web UI exposes the full reasoning process through four dedicated tabs:

#### Citations
Every answer is grounded in source documents. Each citation shows:
- The exact **file name** the answer came from
- The **verbatim quote** from that document

No hallucinations — every claim is traceable.

```
Source 1: deployment-process.md
┌────────────────────────────────────────────┐
│ "Our automated deployment runs at 03:47    │
│  UTC to avoid conflicts with the nightly   │
│  database backup window..."                │
└────────────────────────────────────────────┘
```

#### Statistics
Full transparency on what the agent did and what it cost:

| Metric | Description |
|--------|-------------|
| Time | Total query duration |
| Agent Turns | How many reasoning steps the agent took |
| Input Tokens | Tokens sent to the model |
| Output Tokens | Tokens generated by the model |
| Estimated Cost | Per-query cost (typically $0.0001–$0.001) |

#### Tool Execution
See exactly which tools the agent called and in what order:
...
Completed in 2.5s (3 agent turns)
```

This mirrors how Claude/Cursor exposes their tool calls — full observability into the agent's reasoning chain.

#### Answer
Clean markdown answer synthesized from all retrieved context, always backed by the citations tab.
